...
실행
    python etl/scripts/02_make_market_snapshot_2023_all.py --delay 0.5

    - 기존에 저장된 market_snapshot.csv를 무시하고 처음부터 다시 받고 싶으면:
    python etl/scripts/02_make_market_snapshot_2023_all.py --overwrite
...
DEFAULT_INPUT = PROJECT_ROOT / "etl" / "processed" / "2023" / "financial_base.csv" 
DEFAULT_OUTPUT = PROJECT_ROOT / "etl" / "processed" / "2023" / "market_snapshot.csv" # 성공한 기업
DEFAULT_FAILED = PROJECT_ROOT / "etl" / "processed" / "2023" / "market_snapshot_failed.csv" # 실패한 기업(제무제표에 아무 정보가 없는 경우)
# 한국투자증권 API 키 읽음
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="KIS 2023 통합 대상 종목 현재가/시가총액 수집")
