"""
OpenDART 2024년 연결 재무제표 원본을 일반/금융기타/은행/보험/증권까지 통합해
valuation 계산용 financial_base.csv를 생성

입력 기본 경로
- etl/raw/dart/2024/bs/{general,finance,bank,insurance,securities}/*.txt
- etl/raw/dart/2024/pl/{general,finance,bank,insurance,securities}/*.txt

출력 기본 경로
- etl/processed/2024/financial_base.csv

실행
    python etl/scripts/01_make_financial_base_2024_all.py
...
RAW_ROOT = PROJECT_ROOT / "etl" / "raw" / "dart" / "2024" # 원본 데이터 폳더
PROCESSED_ROOT = PROJECT_ROOT / "etl" / "processed" / "2024"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="OpenDART 2024 통합 financial_base 생성")
    parser.add_argument("--raw-root", type=Path, default=RAW_ROOT, help="etl/raw/dart/2024 경로")
...
def main() -> int:
    ...
    print("2024년 일반+금융업 통합 financial_base 생성 시작")
    ...
    # 핵심 계산에 필요한 자본총계가 없거나 종목코드가 없는 행은 제외
