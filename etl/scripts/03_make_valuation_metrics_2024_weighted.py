"""
2024년 통합 financial_base.csv와 한국투자증권 market_snapshot.csv를 결합해
PER/PBR/ROE/부채비율을 계산하고, 업종별 가중치를 적용한 valuation_score를 생성

입력 기본 경로
- etl/processed/2024/financial_base.csv
- etl/processed/2024/market_snapshot.csv

출력 기본 경로
- etl/processed/2024/valuation_metrics.csv
- etl/processed/2024/market_snapshot_missing_symbols.csv

실행
    python etl/scripts/03_make_valuation_metrics_2024_weighted.py

    - 백엔드에 바로 저장
    python etl/scripts/03_make_valuation_metrics_2024_weighted.py --copy-to-backend
...
PROCESSED_ALL = PROJECT_ROOT / "etl" / "processed" / "2024" 
...
DEFAULT_BACKEND_OUTPUT = PROJECT_ROOT / "backend" / "src" / "main" / "resources" / "data" / "valuation_metrics_2024.csv"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="2024년 업종 가중 valuation_metrics 생성")
...
def main() -> int:
    args = parse_args()
    print("2024년 업종 가중 valuation_metrics 생성 시작")
