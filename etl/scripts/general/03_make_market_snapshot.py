...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="KIS 전체 종목 현재가/시가총액 수집")
...
def normalize_market_cap(raw_market_cap: float | None, price: float | None, listed_shares: float | None) -> float | None:
    """
    KIS 주식현재가 시세 output의 hts_avls는 보통 '억원' 단위로 제공됩니다.
    기존 샘플 CSV는 원 단위 market_cap을 사용하므로, 작은 값은 억 원 → 원으로 변환합니다.
    hts_avls가 없으면 상장주식수 * 현재가로 보정합니다.
    """
